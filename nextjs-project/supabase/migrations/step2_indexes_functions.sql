-- Step 2: Create Indexes and Functions for RAG System
...
ALTER TABLE article_summaries ADD COLUMN IF NOT EXISTS search_vector tsvector GENERATED ALWAYS AS (
  to_tsvector('english',
    coalesce(title, '') || ' ' ||
    coalesce(summary, '') || ' ' ||
...
CREATE INDEX IF NOT EXISTS idx_article_summaries_search ON article_summaries USING GIN(search_vector);
